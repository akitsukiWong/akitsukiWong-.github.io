...
这条语句先用`var`命令，声明了变量`a`，然后将1 + 3的运算结果赋值给变量`a`。
...
a = 1;
```
最后的结果是显示`undefined`，表示变量a已声明，但还未赋值。
...
上面代码的原意是，当`x`等于`y`的时候，才执行相关语句。但是，不小心将严格相等运算符写成赋值表达式，结果变成了将`y`赋值给变量`x`，再判断变量x的值（等于2）的布尔值（结果为`true`）。
